...
import type { EvaluationRunResult, FixtureResult, ExpectationResult } from "./types";
...
function formatSummaryStats(result: EvaluationRunResult): string {
  ...
  const passRateColor = result.passRate === 1
    ? Colors.green
    : result.passRate >= 0.5
      ? Colors.yellow
      : Colors.red;
...
export function formatEvaluationSummary(
  ...
  if (failedResults.length > 0) {
    ...
    failedResults.forEach((fixtureResult) => {
      ...
      lines.push(formatFixtureResult(fixtureResult, originalIndex, result.totalFixtures));
    });
    lines.push("");
  }
...
export function formatCompactSummary(result: EvaluationRunResult): string {
  ...
  const status = result.passRate === 1
    ? colorize("PASS", Colors.green)
    : colorize("FAIL", Colors.red);
